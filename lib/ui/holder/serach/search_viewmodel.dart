...
import 'package:final_project_team02/ui/holder/serach/search_data/items_photo.dart'; // 아이템 사진 데이터를 포함하는 클래스 임포트
...
class SearchViewmodel extends StateNotifier<SearchModel?> {
  final mContext = navigatorKey.currentContext; // 현재 네비게이션 컨텍스트를 가져옴
  ...
  SearchViewmodel(super.state, this.sessionData)
      : searchRepo =
            SearchRepo(); // 초기화 리스트를 사용하여 부모 클래스의 상태 초기화 및 searchRepo 인스턴스화
...
final searchProvider =
  ...
  return SearchViewmodel(null, sessionData)
    ..notifyInit(); // SearchViewmodel 인스턴스를 생성하고 notifyInit 메소드를 호출하여 초기화
});
